...
# VQC-like parameter integration (simple rotation gates for demonstration, scaled for higher dim)
def apply_vqc_to_phi(Phi, theta, phi_param, scale_param, dim):
    ...
    # Extend to full dim using kron with eye(dim//2)
    eye = torch.eye(dim//2, dtype=torch.float64)
...
num_iterations = 500
learning_rate = 0.01
dim = 16  # Extended to 16 for higher-dim verification (original 8)

# Initial Phi (complex vector, scalable dim)
Phi_init = torch.complex(torch.randn(dim, dtype=torch.float64), torch.randn(dim, dtype=torch.float64))  # Scalable dim
...
for iter in range(num_iterations):
    ...
    # Apply VQC to Phi
    Phi = apply_vqc_to_phi(Phi_init, vqc_theta, vqc_phi, scale_param, dim)
...
Phi_final = apply_vqc_to_phi(Phi_init, vqc_theta, vqc_phi, scale_param, dim)
