...
|---|---|---|
| **Top-Left: Z₃ Vacuum Lattice** | 44 colored rays on a dark sphere. Gold thick lines cluster at the "Democratic pole." Cyan lines scatter around (Hybrid family). Red lines lie near the equator. Three dashed axes = flavor coordinates | This is the paper's core mathematical object — the full set of 46 lattice vectors. They are all the "vacuum directions" generated by the Z₃ algebra in 3D. Color tells you which symmetry class (S₃×Z₂ orbit) each vector belongs to |
| **Top-Right: PMNS Mixing Matrix** | 3 thick colored rods (gold = ν₂, cyan = ν₁, red = ν₃), each with dashed lines dropping down to the three flavor axes | Each rod is a mass eigenstate. The dashed projection lines show "how much electron/muon/tau neutrino is in this mass state." Those projection values assembled together ARE the PMNS matrix. sin²θ₁₂ reads off as 0.3092 — matching JUNO to 8 parts per million |
| **Bottom-Left: Oscillation Trajectory** | A golden spiral winding through the space between three flavor axes. Colored dots along the coil mark phase advance | A pure electron neutrino at t=0 oscillates between all three mass states as it travels — tracing a golden spring in flavor space. Each full turn, the detector sees it as a different flavor mix |
| **Bottom-Right: Mass Hierarchy (IO)** | Three colored towers of different heights. Cyan tower = tallest (ν₁ heaviest). Gold = shortest (ν₂ lightest). Red = middle (ν₃) | The framework's hardest falsifiable bet — Inverted Ordering. This is the opposite of what most global fits currently favor. The base reads "JUNO will decide" |

**Controls**: Drag = rotate. Scroll = zoom. Right-drag = pan. Hover any point for details.

---

**In short: the README tells you what the theory says. The 3D HTML lets you see what the theory looks like.**
