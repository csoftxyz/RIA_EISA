...
- **`z3_lattice.py`** (Original, June 2026) — First-generation lattice generator. Stores only normalized cross products; produces a truncated 6-shell lattice (L²=0,1,2,6,18,54) with a spurious zero vector. Missing democratic shells L²=3,27,243 and higher root shells L²=162,486. Retained for historical comparison — see `z3_lattice_A.py` for the paper-verified version.
- **`z3_lattice_A.py`** ★ (Corrected, June 2026) — Verified lattice generator matching both papers (Zhang2026Symmetry + PLB-S-26-02213). Fixes two bugs vs. the original: (i) stores both raw AND normalized cross products (`new.extend([cr, cr/norm])` instead of `new.append(cross/norm)` only), which naturally generates the democratic [1,1,1] (L²=3) and all higher shells; (ii) does not separately re-normalize already-normalized vectors, preventing near-zero float debris. Output: full 10-shell structure — 6 root shells (L²=2,6,18,54,162,486; 6 vectors each = K₂,₂,₂ octahedron, scaled ×√3), 3 democratic shells (L²=3,27,243; 1 vector each along [111], powers of 3), plus the 5-vector basis shell (L²=1). All 44 vectors verified. See `z3_lattice_A_output_log.txt` for the full annotated output.
...
- **`z3_44_lattice_visualizer.html`** ★ (Interactive 3D — New June 2026) — Browser-based 3D explorer of the complete 44-vector lattice with toggleable layers. Red octahedron (K₂,₂,₂ at L²=2), orange scaled octahedra (L²=6 through 486), gold democratic axis [111], cyan A₂ hexagon projection, all 44 vectors. Keyboard shortcuts (1–4 toggle layers, A=all vectors, R=auto-rotate). Self-contained HTML, Three.js CDN. Drag/scroll/zoom.
