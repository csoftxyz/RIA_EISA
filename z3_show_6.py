...
# 修复RG sympy
mu = sp.symbols('mu')
ln_mu = sp.symbols('ln_mu')
alpha, b2, b1 = sp.symbols('alpha b_2 b_1')
sin2_theta = sp.symbols('sin^2_theta_W')
rg_eq = sp.Eq(sp.Derivative(sin2_theta, ln_mu), (alpha / (2 * sp.pi)) * (b2 - b1) * sin2_theta * (1 - sin2_theta))
#ax2.text(0.05, 0.05, f"RG: {sp.latex(rg_eq)}", transform=ax2.transAxes, fontsize=10, bbox=dict(facecolor='white', alpha=0.8))
